...
class UnitTest {
 private:
  void test_attack_setter_getter() {
    ...
    int strength_player = player.get_monster_list()[player.get_current_monster()]->get_strength();
    if(strength_player == 25){
        cout<<"Test 2 (passed) - Get player strength  / Get current monster / get monster list"<<endl;
    }else{
        cout<<"Test 2 (failed) - Get player strength / Get current monster / get monster list "<<endl; 
    }    
    ...
    if(machine.get_monster()->get_health() == (400-25)){
        ...
        cout<<"Test 4 (failed) - Player attack and Machine take attack "<<endl; 
        //if passed meaning get_monster_list(), get_current_monster() and get_strength() also passed.
    }    
    int strength_machine = machine.get_strength();
    player.take_attack(strength_machine);
    if(player.get_monster_list()[player.get_current_monster()]->get_health() == (160 - 10)){
        cout<<"Test 5 (passed) - Machine attack and Player take attack "<<endl;
    }else{
        cout<<"Test 5 (failed) - Machine attack and Player take attack"<<endl;
    }
    player.reset();
    if(player.get_monster_list()[player.get_current_monster()]->get_health() == 160){
  ...
  void test_level_up() {
    ...
    if (player.get_monster_list()[player.get_current_monster()]->get_monster_level() != 2){
